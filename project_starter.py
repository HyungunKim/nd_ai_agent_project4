                    ...
                    for order_result in order_results:
                        if order_result.item_name == item_name and order_result.status == "Insufficient stock":
                            ...
                            transaction_id = create_transaction(
                                ...
                                date=(datetime.strptime(supplier_delivery_date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d"),
                            )
...
order_agent = ToolCallingAgent(model=model,
                         ...
                         When processing order with this agent, you should provide the order due date, item_name, quantity, total price for that item (This is usually generated by the quote agent).
                         If the item is currently being restocked, the order will be processed if it is restocked before due date. (this is checked by get_supplier_delivery_date)
                         """,
                               max_tool_threads=1)
